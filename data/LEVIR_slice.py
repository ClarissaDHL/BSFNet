...
class LEVIR_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        name = datafiles['name']
        if self.transform_med is not None:
...
if __name__ == "__main__":
    data_root = '/home/dhl/PycharmProjects/LEVIR_CD'
    train_list = os.path.join(data_root, 'train.txt')
    test_list = os.path.join(data_root, 'test.txt')
    trainval_list = os.path.join(data_root, 'traintest.txt')
    inform_data_file = os.path.join('./inform', 'LEVIR_inform_whole.pkl')
    ...
    trainloader = data.DataLoader(
        LEVIR_Dataset(data_root, train_list, crop_size=(256,256),
                      img1_mean=datas['img1_mean'], img2_mean=datas['img2_mean']),
        batch_size=4, shuffle=True, num_workers=4,
        pin_memory=True, drop_last=True)

    for idx, batch in enumerate(trainloader):
        img1, img2, label, name, h, w = batch
        print('load success')
