...
def eval_image_rewrite(gt_image, prob, cl_index):
    ...
    # gt_true_num = np.sum(gt_image[:,:]==255)
    # gt_false_num = np.sum(gt_image[:,:]==0)
    # cl_true_num = np.sum(cl_gt[:,:]==True)
    # cl_false_num = np.sum(cl_gt[:,:]==False)
...
def save_PTZ_metric2disk(metrics, save_path):
    ...
    OA_ = list(metrics['metric']['OA'])
    try:
        iu = metrics['metric']['iu']
    except KeyError:
        iu = 0.0
    cont_conv_mean = metrics['contrast_conv']

    metric_ = {'recall': recall_, 'precision': precision_, 'f-score': f_score, 'iu': iu, 'OA':OA_,
               'contrast_conv': cont_conv_mean}
    file_ = open(save_path + '/metric.json', 'w')
    file_.write(json.dumps(metric_, ensure_ascii=False, indent=2))
    file_.close()

def save_PTZ_metric2(metrics, save_path):
    import json
    # metric_dict= {}
    recall_ = list(metrics['metric']['recall'])
    precision_ = list(metrics['metric']['precision'])
    ...
    try:
        ...
        iu = 0.0
    cont_1x_mean = metrics['contrast_1x']
    cont_2x_mean = metrics['contrast_2x']
    cont_8x_mean = metrics['contrast_8x']
    metric_ = {'recall': recall_, 'precision': precision_, 'f-score': f_score, 'iu': iu, 'OA':OA_,
               'contrast_1x': cont_1x_mean, 'contrast_2x':cont_2x_mean, 'contrast_8x':cont_8x_mean}
    file_ = open(save_path + '/metric.json', 'w')
    file_.write(json.dumps(metric_, ensure_ascii=False, indent=2))
    file_.close()
...
def RMS_Contrast(dist_map):
    ...
    mean = np.mean(dist_map_l, axis=0) + 1e-7
